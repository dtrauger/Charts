...
open class BarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
   ...
   @objc  open func setVisibleXRangeMinimum(_ minXRange: Double)
    ...
    @objc open override var chartYMax: Double
    {
        return max(leftAxis._axisMaximum, rightAxis._axisMaximum)
    }

    @objc open override var chartYMin: Double
    {
    ...
    /// backwards.
    @objc open func getTransformer(forAxis axis: YAxis.AxisDependency) -> Transformer
    {
        if axis == .left
        {
    ...
    @objc open override var maxVisibleCount: Int
    {
        get
        {
    ...
    @objc open func isInverted(axis: YAxis.AxisDependency) -> Bool
    {
        return getAxis(axis).isInverted
    }
    
    /// - returns: The lowest x-index (value on the x-axis) that is still visible on he chart.
    @objc open var lowestVisibleX: Double
    {
        var pt = CGPoint(
    ...
    @objc open var highestVisibleX: Double
    {
        var pt = CGPoint(
